# Manual de Referencia SQL Server (Transact-SQL)

Este documento explica los comandos y funciones utilizados en el script de clase, cubriendo desde la creación de bases de datos hasta consultas complejas con agrupaciones.
...
SELECT 
    precio_unitario,
    cantidad,
    precio_unitario * cantidad AS total,       -- Multiplicación
    precio_unitario * cantidad * 0.16 AS iva,  -- Cálculo de IVA
    UPPER(nombre_producto) AS nombre_mayus,    -- Función de texto
    YEAR(fecha_venta) AS anio_venta,           -- Función de fecha
    CONCAT(nombre, ' ', apellido) AS nombre_completo -- Concatenación
FROM ventas;
```